...
var fakeDay = new Date("2018-05-18T12:59:00Z");
...
var temp1ArrHour = new Array();
var temp2ArrHour = new Array();

//variables for today chart
var timeArrDay = new Array();
var temp1ArrDay = new Array();
var temp2ArrDay = new Array();

//variables for this week chart
var timeArrThisWeek = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"];
var temp1ArrThisWeek = new Array();
var temp2ArrThisWeek = new Array();

//variables for this month chart
var timeArrThisMonth = new Array();
var temp1ArrThisMonth = new Array();
var temp2ArrThisMonth = new Array();
...
ourRequest.open('GET', 'https://lkw6ackrp6.execute-api.us-west-2.amazonaws.com/SmartSenseMongoREST');//set HTTP request to a GET REST API
...
ourRequest.onload = function(){//loads data that will be used for charts
  ...
  for (var key in aggregatedDayDataById_minute.SmartSense0001){
      ...
      temp1ArrHour.push(aggregatedDayDataById_minute.SmartSense0001[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }

  for (var key in aggregatedDayDataById_minute.SmartSense0002){
      temp2ArrHour.push(aggregatedDayDataById_minute.SmartSense0002[key]);
  }

  //console.log for troubleshooting purposes
  console.log(temp1ArrHour);
  console.log(temp2ArrHour);
  ...
  for (var key in aggregatedDayDataById_Hour.SmartSense0001){
      ...
      temp1ArrDay.push(aggregatedDayDataById_Hour.SmartSense0001[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }

  for (var key in aggregatedDayDataById_Hour.SmartSense0002){
      temp2ArrDay.push(aggregatedDayDataById_Hour.SmartSense0002[key]);
  }

  //console.log for troubleshooting purposes
  console.log(temp1ArrDay);
  console.log(temp2ArrDay);
  ...
  for (var key in aggregatedThisWeeksDataById_Date.SmartSense0001){
      ...
      temp1ArrThisWeek.push(aggregatedThisWeeksDataById_Date.SmartSense0001[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }

  for (var key in aggregatedThisWeeksDataById_Date.SmartSense0002){
      temp2ArrThisWeek.push(aggregatedThisWeeksDataById_Date.SmartSense0002[key]);
  }

  //console.log for troubleshooting purposes
  console.log(temp1ArrThisWeek);
  console.log(temp2ArrThisWeek);
  ...
  for (var key in aggregatedThisMonthsDataById_Date.SmartSense0001){
      ...
      temp1ArrThisMonth.push(aggregatedThisMonthsDataById_Date.SmartSense0001[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }

  for (var key in aggregatedThisMonthsDataById_Date.SmartSense0002){
      temp2ArrThisMonth.push(aggregatedThisMonthsDataById_Date.SmartSense0002[key]);
  }

  //console.log for troubleshooting purposes
  console.log(temp1ArrThisMonth);
  console.log(temp2ArrThisMonth);
  ...
  for (var key in aggregatedTodaysDataByHour){
      ...
      todayComparison.push(aggregatedTodaysDataByHour[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }
  ...
  for (var key in aggregatedThisWeeksDataByDate){
      ...
      var lastTwoInt = parseInt(lastTwo)//converts the value to an int
      //timeArrThisWeek.push(lastTwoInt);//pushes this value into the array for the time axis
      thisWeekComparison.push(aggregatedThisWeeksDataByDate[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }
  ...
  for (var key in aggregatedThisMonthsDataByDate){
      thisMonthComparison.push(aggregatedThisMonthsDataByDate[key]);//pushes the temperature values from sensor 1 into the the relevent temp axis
  }
  ...
  console.log("latest reading: "+ sensorData[sensorData.length-1].temperature+"C");
  var latestReading = sensorData[sensorData.length-1].temperature;
...
function hourChart() {
      ...
      for(var j=0; j<thisMonthComparison.length; j++){//loop that inputs data into chart
        data.addRows([
          [timeArrHour[j], temp1ArrHour[j], temp2ArrHour[j]]
        ]);
      }
...
function dayChart() {
      ...
      for(var j=0; j<temp1ArrDay.length; j++){//loop that inputs data into chart
        data.addRows([
          [timeArrDay[j], temp1ArrDay[j], temp2ArrDay[j]]
        ]);
      }
...
function thisWeekChart() {
      ...
      for(var y=0; y<temp1ArrThisWeek.length; y++){//loop that inputs data into chart
        data.addRows([
          [timeArrThisWeek[y], temp1ArrThisWeek[y], temp2ArrThisWeek[y]]
        ]);
      }
...
function thisMonthChart() {
      ...
      for(var j=0; j<temp1ArrThisMonth.length; j++){//loop that inputs data into chart
        data.addRows([
          [timeArrThisMonth[j], temp1ArrThisMonth[j], temp2ArrThisMonth[j]]
        ]);
      }
...
function aggregateDataByDate(data){
  var meanTempByDate = d3.nest()
    .key(function(d) { return d.date_time_recorded; })
    .rollup(function(v) { return d3.mean(v, function(d) { return d.temperature; }); })
    .object(data);
  //console.log(JSON.stringify(meanTempByDate));
  //return (JSON.stringify(meanTempByDate));
  //console.log(meanTempByDate);
  return meanTempByDate;

}

//aggregates data by sensor value AND by date then returns in JSON form
function aggregateDataById_Date(data){
  var totalTempById_Date = d3.nest() //delcare new arrays name and initialise the 'd3' nest function
    ...
    .rollup(function(v) { return d3.mean(v, function(d) { return d.temperature; }); })//rollup function gets the mean of all variables with the same date
    .object(data);
  //console.log(JSON.stringify(totalTempById_Date));
  return totalTempById_Date;
  //console.log(totalTempById_Date);
}

//aggregates data by date then returns in JSON form
function aggregateDataByHour(data){
  var meanTempByHour = d3.nest()
    .key(function(d) { return d.hour; })
    .rollup(function(v) { return d3.mean(v, function(d) { return d.temperature; }); })
    .object(data);
  //console.log(JSON.stringify(meanTempByHour));
  //return (JSON.stringify(meanTempByHour));
  var sorted = JSON.parse(JSON.sortify(meanTempByHour));//JSON orders alphabetically but we need to order numberically so we use the library "sortify" 
...
function aggregateDataById_Hour(data){
  var meanTempById_Hour = d3.nest()
    .key(function(d) { return d.sensor_id; })
    .key(function(d) { return d.hour; })
    .rollup(function(v) { return d3.mean(v, function(d) { return d.temperature; }); })
    .object(data);
  //console.log(JSON.stringify(meanTempById_Hour));
  //return (JSON.stringify(meanTempById_Hour));
  var sorted = JSON.parse(JSON.sortify(meanTempById_Hour));//JSON orders alphabetically but we need to order numberically so we use the library "sortify"

  return sorted;

  //console.log(meanTempById_Hour);
}

//aggregates data by date then returns in JSON form
function aggregateDataByMinute(data){
  var meanTempByMinute = d3.nest()
    .key(function(d) { return d.minute; })
    .rollup(function(v) { return d3.mean(v, function(d) { return d.temperature; }); })
    .object(data);
  //console.log(JSON.stringify(meanTempByMinute));
  //return (JSON.stringify(meanTempByMinute));
  var sorted = JSON.parse(JSON.sortify(meanTempByMinute));//JSON orders alphabetically but we need to order numberically so we use the library "sortify" 
...
function aggregateDataById_Minute(data){
  var meanTempById_Minute = d3.nest()
    .key(function(d) { return d.sensor_id; })
    .key(function(d) { return d.minute; })
    .rollup(function(v) { return d3.mean(v, function(d) { return d.temperature; }); })
//    .sortKeys(d3.ascending)
  ...
  var sorted = JSON.parse(JSON.sortify(meanTempById_Minute));
